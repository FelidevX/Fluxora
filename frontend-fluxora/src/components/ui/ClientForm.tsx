...
const MapSelector = dynamic(() => import('./MapSelector'), {
  ssr: false,
  loading: () => <div className="h-64 bg-gray-100 rounded-md flex items-center justify-center">Cargando mapa...</div>
});
...
const ClientForm: React.FC<ClientFormProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE}/api/clientes/clientes`, {
        method: 'POST',
        headers: {
      ...
      alert(`Error al registrar cliente: ${error instanceof Error ? error.message : 'Error desconocido'}`);
    } finally {
    ...
    <div className="bg-white p-6 rounded-lg border-2 border-blue-500 max-w-2xl">
      <div className="flex items-center mb-4">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">Ingrese los nuevos datos del cliente</p>
        </div>
      </div>
