 ...
 */
export async function buscarMateriaPorNombre(nombre: string): Promise<MateriaPrima[]> {
  try {
    const response = await fetch(
      `http://localhost:8080/api/inventario/materias-primas/buscar/${encodeURIComponent(nombre)}`
    );
...
export function encontrarSimilitud(nombre1: string, nombre2: string): number {
  ...
  const similitud = coincidencias / Math.max(palabras1.length, palabras2.length);
...
export async function generarSugerenciasReparacion(
  ...
  for (const ingrediente of ingredientesRotos) {
    ...
    for (const materia of todasLasMaterias) {
      ...
      if (similitud > mejorSimilitud && similitud > 0.6) { // 60% de similitud mínima
...
export async function repararRecetaAutomaticamente(
  ...
  if (!receta.ingredientes || !Array.isArray(receta.ingredientes)) {
    return { reparada: false, cambiosRealizados: [], recetaActualizada: receta };
  }
...
export async function repararTodasLasRecetasAutomaticamente(
  ...
  try {
    ...
    const recetasGuardadas = localStorage.getItem('fluxora_recetas'); // Usar la clave correcta
    ...
    for (const receta of recetas) {
      const resultado = await repararRecetaAutomaticamente(receta, todasLasMaterias, umbralSimilitud);
      ...
      if (resultado.reparada) {
        ...
        // Actualizar la receta en el array
        const index = recetas.findIndex((r: any) => r.id === receta.id);
        if (index !== -1) {
          recetas[index] = resultado.recetaActualizada;
        }
      }
    }
    
    // Guardar las recetas actualizadas
    if (recetasReparadas > 0) {
      localStorage.setItem('fluxora_recetas', JSON.stringify(recetas)); // Usar la clave correcta
    }
